...
public class App {
    ...
    private void initialize() {
        ...
        try {
            List<String> membersData = FileStorageService.readContentsOfFile(FILE_BASE_PATH + "members.dat");
            for (String data : membersData) {
                String[] fields = data.split(FIELD_DELIMITER);
                Member member = new Member(fields[0], fields[1]);
                List<CatalogItem> items = processItems(FILE_BASE_PATH + fields[2]);
    ...
    private List<CatalogItem> processItems(String filename) {
        ...
        try {
            ...
            for (String item : itemData) {
                ...
                if (fields.length != 4) {
                    throw new RuntimeException("Invalid number of fields in: " + filename);
                }
                    switch (fields[0]) {
                        ...
                        default:
                            System.out.println("Invalid catalog item format.");
                            System.exit(1);
                    }
